 ...
 * All user and transaction data are stored in CSV files.
 */
public class BankingSystem {
    private static final String ADMIN_USERNAME = "admin";
    private static final String ADMIN_PASSWORD = "password";

    private static final String ACTIVE_ACCOUNTS_FILE = "active_accounts.csv";
    private static final String PENDING_ACCOUNTS_FILE = "pending_accounts.csv";
    ...
    public static void main(String[] args) {
        ...
        system.loadData(); // Load data from files
        system.run();
    }
    ...
    private void userLogin(Scanner scanner) {
        ...
        if (user != null && user.verifyPassword(password)) {
            user.loadTransactions(); // Load transactions for this user
            ...
            try {
                switch (choice) {
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
                        user.saveTransactions(); // Save transactions before logging out
                        return;
                    default:
                        System.out.println("Invalid choice. Please try again.");
                }
    ...
    /**
     * Loads active accounts and pending accounts from their respective files.
     */
    private void loadData() {
        loadUsersFromFile(ACTIVE_ACCOUNTS_FILE, activeAccounts, false);
        loadUsersFromFile(PENDING_ACCOUNTS_FILE, null, true);
    }

    private void loadUsersFromFile(String fileName, Map<String, User> userMap, boolean isPending) {
        try (BufferedReader reader = new BufferedReader(new FileReader(fileName))) {
            String line;
            while ((line = reader.readLine()) != null) {
                User user = User.parseUser(line);
                if (isPending) {
                    pendingAccounts.add(user);
                } else {
                    activeAccounts.put(user.getUsername(), user);
                }
            }
    ...
    /**
     * Saves active accounts and pending accounts to their respective files.
     */
    private void saveData() {
        saveUsersToFile(ACTIVE_ACCOUNTS_FILE, activeAccounts.values());
        saveUsersToFile(PENDING_ACCOUNTS_FILE, pendingAccounts);
    }

    private void saveUsersToFile(String fileName, Collection<User> users) {
